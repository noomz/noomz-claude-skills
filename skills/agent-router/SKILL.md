...
1. Read the quota line injected at session start.
...
The picker reads `aub --cached --json --provider all` by default. `--fresh` omits `--cached`. Missing or failing `aub` is non-fatal: scripts exit 0 and print nothing.
...
`quota_snapshot.py` is the SessionStart hook. `agent_call_guard.py` is the non-blocking PreToolUse hook for `Agent|Task|Workflow`. Both are safe to install: they emit no output on malformed input or unavailable quota data. The guard only reads the picker stamp and keeps a per-session call count.
