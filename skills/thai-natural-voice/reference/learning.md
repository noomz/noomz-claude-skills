...
This pipeline acquires a source, extracts broad voice traits, and stores them. It does not schedule re-learning, sync profiles between machines, share or publish profiles, or fetch anything the user did not name in the current request.
...
~/.claude/thai-voice/
  ...
  profiles/
    ...
    <slug>-learned.md     # one profile per file
```

Config, created on first learn. Every key optional; the listed value is the default.
...
`on_conflict: version` writes `<slug>-learned-2.md`, `-3`, and so on, leaving earlier profiles intact.

Learned ids end in `-learned`, for the same reason bundled ids end in `-inspired`: the suffix marks an approximation so a learned profile can never read as an official house style.

## Selector placement

A learned profile is selected by `voice: <slug>-learned`, or by any alias recorded in its own `Aliases` field.
...
If the name already resolves to a bundled alias, say so and offer two paths: use the bundled preset, or learn a fresh profile anyway.
...
3. **Prose** — the samples are continuous writing, not a cookie banner, consent wall, paywall stub, login page, navigation list, or error body. A cookie wall returns text and will otherwise be learned as a voice.
...
Write `<slug>-learned.md`, append a row to `index.md`, report the path. If `auto_apply` is true, apply the profile to the next request in the session and say so.
...
| Samples are not predominantly Thai | Stop. Report the detected language. Offer to learn structural traits only, with every Thai-specific control left `not observed`. |
...
- Reuse the `-inspired` schema so learned and bundled profiles are interchangeable at the selector.
- Record provenance in every profile: source label, URLs, date learned, sample count, word count.
...
- Don't let a learned profile relax the skill's invariants. No affiliation claims, no bylines, no copied slogans, hashtags, or sign-offs, no reproduction of a passage's distinctive sequence. Learning from a source never upgrades the output's authority.
- Don't treat `reference/research.md` as a sample source. It is provenance for the bundled profiles; sources researched there are not sources this user learned.
- Don't fill an unobserved field.

## Verification

After a learn completes, confirm all of these:
...
Round-trip test: run the same drafting request twice, once with the default `คุยรู้เรื่อง` and once with `voice: <slug>-learned`. The outputs must differ in structure or surface controls, not only in wording. Identical outputs mean the profile carries no usable signal — report that rather than leaving it in place.
